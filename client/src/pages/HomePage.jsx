...
export default function HomePage() {
  ...
  const resetAllFilters = () => {
    ...
    setCuisine("All");
    setVegFilter("all");
    // Explicitly fetch with empty params to avoid stale state issues
    fetchFoods("");
  };
  ...
  return (
    <div className="max-w-md md:max-w-7xl mx-auto min-h-screen px-4 md:px-8 py-4 pt-16 bg-[var(--bg-primary)] text-[var(--text-primary)] transition-colors duration-300">
      {toast && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[110] bg-[var(--text-primary)] text-[var(--bg-primary)] px-6 py-3 rounded-full flex items-center space-x-3 shadow-2xl animate-slide-up border border-[var(--border-color)]">
      ...
      <div className="relative overflow-hidden -mx-4 md:mx-auto -mt-4 md:mt-2 px-6 md:px-12 pt-10 pb-16 rounded-b-[3rem] md:rounded-[3rem] shadow-sm mb-8">
          ...
          <div className="relative z-10 md:max-w-2xl mx-auto">
              <div className="text-center mb-8">
                  <h2 className="text-3xl md:text-5xl font-black text-[var(--text-primary)] tracking-tighter mb-2 leading-tight">
                     Find what you <span className="text-[var(--brand-orange)] drop-shadow-sm">crave</span>
                  </h2>
                  <p className="text-[var(--text-primary)]/70 font-bold text-sm md:text-lg">Discover the best food & drinks in your city</p>
              </div>
      ...
      <div className="mt-8 mb-12">
        <div className="flex items-center justify-between mb-8 px-1">
          ...
          <div className="flex space-x-2">
             <div className="w-8 h-8 rounded-full border border-[var(--border-color)] flex items-center justify-center text-[var(--text-secondary)]">‹</div>
             <div className="w-8 h-8 rounded-full border border-[var(--border-color)] flex items-center justify-center text-[var(--text-secondary)]">›</div>
          </div>
        </div>
        
        <div className="flex overflow-x-auto space-x-6 md:space-x-12 no-scrollbar pb-4 -mx-4 px-4 md:mx-0 md:px-0">
          {CUISINES.map(c => (
            <button
              key={c}
      ...
      {/* ── Section 2: Top Brands (NearbyRestaurants) ── */}
      <div className="relative z-20 md:max-w-7xl md:mx-auto">
      ...
      <div className="mt-16 space-y-8">
        ...
        <div className="pb-32">
          ...
          {loading ? (
            ...
            <div className="text-center py-20 bg-[var(--bg-surface)] rounded-[3rem] border border-[var(--border-color)] shadow-sm">
